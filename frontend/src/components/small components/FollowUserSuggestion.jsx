...
const FollowUserSuggestion = ({currUser}) => {
    ...
    return (
        <div onClick={ (e) => navigateToProfilePage(e, dispatch, navigate, currUser._id) } className='relative flex py-2 hover:bg-[#323333]/60 cursor-pointer'>
            ...
            {
                <div onClick={ (e) => handleFollowOrUnfollow(e, dispatch, currUser._id, whichDivIsActive, setAreDisplayTweetLoading) } className='absolute right-5 top-[12px] w-[96px] h-[36px] bg-blue-500 rounded-full overflow-hidden'>
                    <button   
