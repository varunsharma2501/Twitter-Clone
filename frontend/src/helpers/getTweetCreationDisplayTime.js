export const getTweetCreationDisplayTime = (currTweet) => {
    ...
    const tempCreatedOnDate = currTweet?.createdAt?.split('T');
    const tempCreatedOnUTC = currTweet?.createdAt?.split('T');
    
    let tweetCreatedOnDate; 
    let tweetCreatedOnUTC; 

    if(tempCreatedOnDate && tempCreatedOnUTC){
        tweetCreatedOnDate = tempCreatedOnDate[0].split('-').reverse().map( (str) => parseInt(str));
        tweetCreatedOnUTC = tempCreatedOnUTC[1].split(':').map( (str) => parseInt(str));
